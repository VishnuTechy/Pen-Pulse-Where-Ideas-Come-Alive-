...
async function requireAdmin() {
  ...
  if (!data) return [null, new Response(JSON.stringify({ message: 'Unauthorized' }), { status: 401 })]
  if (data.role !== 'admin') return [null, new Response(JSON.stringify({ message: 'Forbidden' }), { status: 403 })]
  return [data, null]
}
